...
const HeaderLink: React.FC<{ item: HeaderItem }> = ({ item }) => {
  ...
  return (
    ...
    >
      <Link href={item.href} className={`text-white flex py-3 font-normal text-midnight_text hover:text-gray dark:text-white dark:hover:text-primary ${path === item.href ? '!text-lightgray' : ' text-black dark:text-white '} ${path.startsWith(`/${item.label.toLowerCase()}`) ? "!text-white " : null}`}>
        {item.label}
        {item.submenu && (
